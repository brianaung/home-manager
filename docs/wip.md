...
### Bootstraping
1. Lauch shell as root user > `sudo -i`.

2. Partition and format disks, then install the OS.
...
### Rebuilding
1. Rebuild the system > `nixos-rebuild switch --flake .#<NIXNAME>`.

2. Generate hashed password > `mkpasswd -m sha-512 <your_password> > /etc/passwordFile`
